...
export default function UpdateStatus() {
  ...
  // const subtaskIndex = params.get("subtaskIndex");
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        const res = await axios.get(`http://10.49.33.16:5656/api/v1/tasks/${taskId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setMessage("");
      let res = await axios.put("http://10.49.33.16:5656/api/v1/tasks/update-status", {
        taskId,
        owner,
        actionItem,
        subtaskIndex,
